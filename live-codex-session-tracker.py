...
def load_model_pricing():
    ...
    if not pricing_file.exists():
        ...
        return {
            "models": {
                "openai": {
                    "gpt-4o": {"name": "GPT-4o", "input": 2.5, "output": 10.0, "cache_read": 1.25}
                }
            },
...
class CodexTracker:
    def __init__(self, project_path="wp-navigator-pro/main"):
        ...
        self.project_abs_path = os.path.abspath(
            os.path.expanduser(f"~/claude-code-tools/lba/plugins/{project_path}")
        )
    ...
    def calculate_cost(self):
        ...
        if not model_pricing:
            model_pricing = {"input": 2.5, "output": 10.0, "cache_read": 1.25}
        ...
        cost_usd = (
            ...
            + (
                self.total_cache_create * model_pricing.get("input", model_pricing["input"])
            )  # Treat cache_create as input
            + (
                self.total_cache_read
                * model_pricing.get("cache_read", model_pricing["input"] * 0.5)
    ...
    def display_stats(self):
        ...
        print(
            f"{Colors.BLUE}║{Colors.NC} {Colors.BOLD}WP Navigator Pro - Live Codex CLI Tracker{Colors.NC}                  {Colors.BLUE}║{Colors.NC}"
        )
        print(
            f"{Colors.BLUE}╚════════════════════════════════════════════════════════════════╝{Colors.NC}"
        )
        ...
        if self.mcp_stats:
            ...
            for server, stats in sorted_servers:
                ...
                print(
                    f"    Calls: {calls:>5}  |  Tokens: {self.format_number(server_tokens):>10}  ({percentage:.1f}%)"
                )

            # Built-in tools summary
            if self.builtin_tool_calls > 0:
                builtin_percentage = (
                    (self.builtin_tool_tokens / total_tokens * 100) if total_tokens > 0 else 0
                )
                print()
                print(f"  {Colors.CYAN}built-in tools:{Colors.NC}")
                print(
                    f"    Calls: {self.builtin_tool_calls:>5}  |  Tokens: {self.format_number(self.builtin_tool_tokens):>10}  ({builtin_percentage:.1f}%)"
                )
        else:
    ...
    def process_line(self, line, debug=False):
        ...
        try:
            ...
            if last_usage and isinstance(last_usage, dict):
                ...
                if debug:
                    self.add_debug(
                        f"✓ Tokens: in={input_tokens}, out={output_tokens}, cache={cached_input}"
                    )

                # Attribute tokens to buffered tool calls
                if self.tool_call_buffer:
                    self._attribute_tokens_to_tools(
                        input_tokens, output_tokens, 0, cached_input, timestamp
                    )
    ...
    def _attribute_tokens_to_tools(
        self, input_tokens, output_tokens, cache_create, cache_read, timestamp
    ):
    ...
    def write_summary(self):
        ...
        summary = {
            ...
            "cost": {"usd": round(actual_usd, 4), "aud": round(actual_aud, 4)},
            ...
            "mcp_servers": {
                server: {
                    ...
                    "percentage": (
                        round((stats["total_tokens"] / total_tokens * 100), 2)
                        if total_tokens > 0
                        else 0
                    ),
                }
            ...
            "builtin_tools": {
                ...
                "percentage": (
                    round((self.builtin_tool_tokens / total_tokens * 100), 2)
                    if total_tokens > 0
                    else 0
                ),
            },
            ...
            if server in self.mcp_tool_stats:
                for tool_name, tool_stats in self.mcp_tool_stats[server].items():
                    tools[tool_name] = {
                        ...
                        "percentage": (
                            round((tool_stats["total_tokens"] / stats["total_tokens"] * 100), 2)
                            if stats["total_tokens"] > 0
                            else 0
                        ),
                    }

            breakdown = {"server": server, "summary": stats, "tools": tools}
                    ...
                    try:
                        with open(file_path, "r") as f:
                            ...
                            if new_content:
                                ...
                                self.add_debug(
                                    f"Found {len(lines)} new lines in {file_path.name[:20]}..."
                                )
                                ...
                                if processed_count > 0:
                                    ...
                                    self.add_debug(
                                        f"✓ Processed {processed_count} messages (+{self.format_number(total)} tokens)"
                                    )
                        ...
                        self.add_debug(
                            f"⚠ Error reading {file_path.name[:20]}: {type(e).__name__}: {str(e)[:40]}"
                        )
                        continue
