# MCP Audit AI Export Schema

**Version**: 1.0.0
**Last Updated**: 2025-12-07
**Status**: Proposed (v0.5.0 target)

This document defines the AI-ready export format generated by `[A] Ask AI` in the TUI or `mcp-audit export ai-prompt` CLI command.
...
The AI CLI agent becomes the analytical layer. MCP Audit provides facts; AI provides interpretation.
...
```bash
# Single session
mcp-audit export ai-prompt --session <id>

# Multiple sessions (comparison)
mcp-audit export ai-prompt --sessions <id1> <id2> <id3>

# Save to file
mcp-audit export ai-prompt --session <id> --out analysis-request.md
```
...
|-------|------|-------------|
| `type` | string | Always `"mcp_audit_ai_summary"` or `"mcp_audit_ai_comparison"` |
...
```json
{
  ...
  "sessions": [
    ...
    {
      "session_id": "mcp-audit-2025-12-06T16-42-00",
      "project": "mcp-audit",
      "platform": "codex-cli",
      "data_quality": "~6%",
...
You will receive telemetry from a single MCP Audit session.
...
You will receive telemetry for multiple MCP Audit sessions.
...
- [AI Prompt Export Spec](ideas/mcp-audit-ai-prompt-export-spec.md) - Original specification
