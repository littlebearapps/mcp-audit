...
class SessionTracker:
    def __init__(self, project_path="wp-navigator-pro/main"):
        ...
        if self.git_metadata['branch']:
            print(f"{Colors.CYAN}[INIT] Git branch: {self.git_metadata['branch']} ({self.git_metadata['commit_short']}){Colors.NC}")
        ...
        if not self.claude_dir.exists():
            ...
            if not found:
                print(f"{Colors.YELLOW}Warning: Project directory not found: {project_dir_name}{Colors.NC}")
                print(f"{Colors.YELLOW}Looking in: {base_dir}{Colors.NC}")
                print(f"{Colors.YELLOW}This may mean no Claude Code sessions exist for this project yet.{Colors.NC}")
        ...
        if len(token_history) >= 3:  # Need at least 3 calls to detect variance
            ...
            if min_tokens > 0:  # Avoid division by zero
                ...
                if variance_ratio > self.HIGH_VARIANCE_RATIO:
                    ...
                    existing = [a for a in self.anomalies['high_variance'] if a['tool'] == tool_name]
                    if not existing:
                        anomaly = {
                            'timestamp': datetime.now().isoformat() + 'Z',
                            'type': 'high_variance',
    ...
    def display_mcp_breakdown(self):
        ...
        for server_name, stats in sorted_servers:
            ...
            print(f"  {server_name:<20} {calls:>3} calls  {tokens_str:>8} tokens  (avg {avg_str}/call)")
            ...
            if server_name in self.mcp_tool_stats:
                ...
                for tool_full_name, tool_stats in sorted_tools:
                    ...
                    tool_short = tool_full_name.split('__')[-1] if '__' in tool_full_name else tool_full_name
                    ...
                    print(f"    └─ {tool_short:<17} {tool_calls:>3} calls  {tool_tokens_str:>8} tokens  ({pct_of_server:.0f}% of server)")

        # Summary line
        print(f"{Colors.YELLOW}───────────────────────────────────────────────────────────────{Colors.NC}")

        # Calculate percentage
        mcp_percentage = (total_mcp_tokens / total_session_tokens * 100) if total_session_tokens > 0 else 0
        ...
        print(f"  {Colors.BOLD}Total MCP usage:{Colors.NC}    {total_mcp_calls:>3} calls  {total_str:>8} tokens  ({mcp_percentage:.0f}% of session)")

        # Built-in tools summary (if any)
        if self.builtin_tool_calls > 0:
            builtin_percentage = (self.builtin_tool_tokens / total_session_tokens * 100) if total_session_tokens > 0 else 0
            ...
            print(f"  Built-in tools:      {self.builtin_tool_calls:>3} calls  {builtin_str:>8} tokens  ({builtin_percentage:.0f}% of session)")
    ...
    def display_stats(self):
        ...
        print(f"{Colors.BLUE}║{Colors.NC} {Colors.BOLD}WP Navigator Pro - Live Session Tracker{Colors.NC}                       {Colors.BLUE}║{Colors.NC}")
        print(f"{Colors.BLUE}╚════════════════════════════════════════════════════════════════╝{Colors.NC}")
        print()
    ...
    def find_project_files(self, verbose=False):
        ...
        if not self.claude_dir.exists():
            if verbose:
                print(f"{Colors.RED}[FILES] Claude directory does not exist: {self.claude_dir}{Colors.NC}")
        ...
        if verbose:
            print(f"{Colors.CYAN}[FILES] Found {len(project_files)} non-empty .jsonl files{Colors.NC}")
                    ...
                    if isinstance(content_block, dict) and content_block.get('type') == 'tool_use':
                        ...
                        if tool_name:
                            ...
                            if mcp_server:
                                ...
                                mcp_tools_used.append((mcp_server, tool_name))  # Store (server, tool) pair
                                if debug:
                                    print(f"  [DEBUG] MCP tool found: {tool_name} (server: {mcp_server})")
                            else:
            ...
            summary = {
                ...
                'builtin_tools': {
                    ...
                    'percentage_of_session': round(
                        (self.builtin_tool_tokens / total_tokens) * 100, 1
                ...
                'redundancy_analysis': {
                    ...
                    'redundancy_ratio': round(
                        len(self.duplicate_calls) / len(self.tool_call_signatures), 3
                    ) if len(self.tool_call_signatures) > 0 else 0,
        ...
        try:
            ...
            for server_name, server_stats in self.mcp_stats.items():
                ...
                for tool_full_name, tool_stats in self.mcp_tool_stats[server_name].items():
                    ...
                    avg_per_call = tool_stats['total_tokens'] // tool_stats['calls'] if tool_stats['calls'] > 0 else 0
                    pct_of_server = round(
                        (tool_stats['total_tokens'] / server_stats['total_tokens']) * 100, 1
                    ) if server_stats['total_tokens'] > 0 else 0
        ...
        if self.mcp_stats:
            ...
            if missing_files:
                ...
                for server in missing_files:
                    ...
                    print(f"{Colors.YELLOW}  • mcp-{server}.json (lost {calls} calls, {tokens:,} tokens){Colors.NC}")
                print(f"{Colors.YELLOW}[WARNING] This may indicate the session was interrupted during cleanup.{Colors.NC}")
                print(f"{Colors.CYAN}[INFO] Run the analyzer to recover data from events.jsonl{Colors.NC}")
            else:
                # All MCP files written successfully
                print(f"{Colors.GREEN}[VALIDATION] ✓ All MCP server files written successfully{Colors.NC}")
                        ...
                        with open(file_path, 'r') as f:
                            ...
                            if new_content:
                                ...
                                if processed_count > 0:
                                    self.add_debug(f"✓ Processed {processed_count} messages (+{self.format_number(self.total_input + self.total_output)} tokens)")
...
Press Ctrl+C to stop tracking and save session data.
    ...
    parser.add_argument('--no-logs', action='store_true',
                        help="Don't save session logs to disk")
