...
class MCPEfficiencyAnalyzer:
    ...
    def load_sessions(self):
        ...
        if not self.sessions_dir.exists():
            print(f"{Colors.RED}Error: Sessions directory not found: {self.sessions_dir}{Colors.NC}")
            return False
        ...
        if not session_folders:
            print(f"{Colors.YELLOW}Warning: No session folders found in {self.sessions_dir}{Colors.NC}")
            return False
    ...
    def _load_session(self, session_folder):
        ...
        if summary_file.exists():
            ...
            if events_file.exists():
                print(f"{Colors.CYAN}Attempting recovery for {session_folder.name} from events.jsonl{Colors.NC}")
    ...
    def _recover_from_events(self, session_folder, session_data):
        ...
        try:
            ...
            if mcp_tools:
                ...
                print(f"{Colors.GREEN}✓ Recovered {len(mcp_tools)} MCP server(s) from events{Colors.NC}")
            else:
                print(f"{Colors.YELLOW}⚠ No MCP tools found in events (session used built-in tools only){Colors.NC}")
    ...
    def calculate_derived_stats(self):
        ...
        for tool_name, stats in self.tool_stats.items():
            ...
            stats['avg_calls_per_session'] = (
                stats['total_calls'] / stats['session_count']
                if stats['session_count'] > 0 else 0
            )
            ...
            if len(stats['token_values']) >= 3:
                ...
                stats['variance_ratio'] = (
                    stats['max_tokens'] / stats['min_tokens']
                    if stats['min_tokens'] > 0 else 0
                )
    ...
    def display_report(self):
        ...
        print(f"{Colors.CYAN}Total Tool Calls:{Colors.NC} {sum(s['total_calls'] for s in self.tool_stats.values()):,}")
        print(f"{Colors.CYAN}Total Tokens:{Colors.NC} {sum(s['total_tokens'] for s in self.tool_stats.values()):,}")
        ...
        for tool_name, stats in outliers['top_expensive']:
            ...
            print(f"{short_name[:50]:<50} {stats['total_calls']:>8,} {stats['total_tokens']:>12,} {stats['avg_tokens_per_call']:>12,}")
        ...
        for tool_name, stats in outliers['top_frequent']:
            short_name = tool_name.split('__')[-1] if '__' in tool_name else tool_name
            print(f"{short_name[:50]:<50} {stats['total_calls']:>8,} {stats['session_count']:>10} {stats['avg_calls_per_session']:>12.1f}")

        # 3. High Average Tokens (>100K)
        if outliers['high_avg_tokens']:
            print(f"\n{Colors.RED}{'─' * 80}{Colors.NC}")
            print(f"{Colors.BOLD}{Colors.RED}⚠️  Tools with High Average Tokens (>{self.HIGH_AVG_THRESHOLD:,}){Colors.NC}")
        ...
        if outliers['high_frequency']:
            print(f"\n{Colors.RED}{'─' * 80}{Colors.NC}")
            print(f"{Colors.BOLD}{Colors.RED}⚠️  Tools with High Call Frequency (>{self.HIGH_FREQ_THRESHOLD} calls/session){Colors.NC}")
            print(f"{Colors.RED}{'─' * 80}{Colors.NC}")
        ...
        if outliers['high_variance']:
            print(f"\n{Colors.YELLOW}{'─' * 80}{Colors.NC}")
            print(f"{Colors.BOLD}Tools with High Token Variance ({self.HIGH_VARIANCE_RATIO}x+ difference){Colors.NC}")
            print(f"{Colors.YELLOW}{'─' * 80}{Colors.NC}")
            ...
            for tool_name, stats in outliers['high_variance']:
                short_name = tool_name.split('__')[-1] if '__' in tool_name else tool_name
                print(f"{short_name[:40]:<40} {stats['min_tokens']:>10,} {stats['max_tokens']:>10,} {stats['variance_ratio']:>8.1f}x")
        else:
    ...
    def export_csv(self, output_file="mcp-efficiency-report.csv"):
        ...
        rows = []
        for tool_name, stats in sorted(self.tool_stats.items(), key=lambda x: x[1]['total_tokens'], reverse=True):
            rows.append({
                'tool_name': tool_name,
                'short_name': tool_name.split('__')[-1] if '__' in tool_name else tool_name,
                ...
                'high_avg_outlier': 'YES' if stats['avg_tokens_per_call'] > self.HIGH_AVG_THRESHOLD else 'NO',
                'high_freq_outlier': 'YES' if stats['avg_calls_per_session'] > self.HIGH_FREQ_THRESHOLD else 'NO',
                'high_variance_outlier': 'YES' if stats['variance_ratio'] > self.HIGH_VARIANCE_RATIO else 'NO',
        ...
        with open(output_path, 'w', newline='') as f:
            fieldnames = [
                'tool_name', 'short_name', 'servers', 'total_calls', 'total_tokens',
                'session_count', 'avg_tokens_per_call', 'avg_calls_per_session',
                'min_tokens', 'max_tokens', 'median_tokens', 'variance_ratio', 'stdev_tokens',
                'high_avg_outlier', 'high_freq_outlier', 'high_variance_outlier',
                'sessions_used'
            ]
