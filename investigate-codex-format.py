...
class CodexInvestigator:
            ...
            for month_dir in sorted(year_dir.iterdir(), reverse=True):
                ...
                for day_dir in sorted(month_dir.iterdir(), reverse=True):
                    ...
                    for file_path in sorted(
                        day_dir.glob("*.jsonl"), key=lambda x: x.stat().st_mtime, reverse=True
                    ):
    ...
    def analyze_entry(self, data, filename, line_num):
        ...
        if len(self.sample_entries[entry_type]) < 2:
            self.sample_entries[entry_type].append(
                {"file": filename, "line": line_num, "data": data}
            )
    ...
    def report_findings(self):
        ...
        if self.token_related_entries:
            print(
                f"{Colors.GREEN}Found {len(self.token_related_entries)} entries with token-related keywords{Colors.NC}\n"
            )
            ...
            print(
                f"{Colors.YELLOW}This suggests tokens might be calculated by ccusage, not stored directly.{Colors.NC}"
            )
        print()
        ...
        if self.token_related_entries:
            ...
            print(
                f"  Next step: Extract token values from {self.token_related_entries[0]['type']} entries"
            )
        else:
